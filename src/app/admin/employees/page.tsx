...
import { Plus, X, Loader2, Users, Search, Mail, Phone, Building2, Eye, UserX, UserCheck } from 'lucide-react'
...
export default function AdminEmployeesPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upsert profile with extra details
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(emp => {
            ...
            return (
              <div key={emp.id} className={cn('card p-5 transition-opacity', !emp.is_active && 'opacity-60')}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-full bg-indigo-500 flex items-center justify-center text-white font-semibold shrink-0 text-sm">
                ...
                {summary ? (
                  <>
                    <div className="grid grid-cols-4 gap-1 mb-3">
                      {[
                        { label: 'Total', value: summary.total_tasks, color: 'text-slate-600 dark:text-slate-300' },
                        { label: 'Done', value: summary.completed_tasks, color: 'text-emerald-600 dark:text-emerald-400' },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-4">
              ...
              <div>
                <label className="label" htmlFor="emp-email">Email Address *</label>
                <input
                  id="emp-email"
                  type="email"
                  className="input"
              ...
              <div>
                ...
                <input
                  ...
                  minLength={8}
                />
                <p className="text-xs text-slate-400 mt-1">Employee will use this to log in</p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting || !form.full_name.trim() || !form.email.trim() || form.password.length < 8}
