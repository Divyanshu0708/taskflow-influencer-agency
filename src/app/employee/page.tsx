...
import { CheckCircle2, Clock, AlertCircle, Circle, Calendar, ArrowRight, LogIn, LogOut, CheckSquare } from 'lucide-react'
...
export default function EmployeeDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [tasksRes, attRes] = await Promise.all([
      supabase.from('tasks').select('*, assignee:profiles!tasks_assigned_to_fkey(id, full_name)')
        .eq('assigned_to', profile.id).order('deadline', { ascending: true }),
  ...
  const handleCheckIn = async () => {
    ...
    if (!attendance) {
      const { data } = await supabase.from('attendance').insert([{
        user_id: profile.id, date: today, check_in: new Date().toISOString()
      }]).select().single()
      if (data) { setAttendance(data); toast.success('Checked in! Have a great day 🎉') }
      ...
      if (data) { setAttendance(data); toast.success('Checked out. See you tomorrow! 👋') }
    }
    setCheckingIn(false)
  }
  ...
  const urgentTasks = tasks.filter(t => t.status !== 'completed' && (t.priority === 'high' || isOverdue(t.deadline, t.status))).slice(0, 5)
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="card p-3 flex items-center gap-3">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {!checkedIn ? 'Not checked in' : !checkedOut ? `In since ${format(new Date(attendance!.check_in!), 'h:mm a')}` : `Done at ${format(new Date(attendance!.check_out!), 'h:mm a')}`}
            </p>
          ...
          {!checkedOut && (
            <button
              ...
              disabled={checkingIn}
              className={cn('btn-primary text-xs px-3 py-2 shrink-0', checkedIn && 'bg-slate-600 hover:bg-slate-700')}
            >
              {checkedIn ? <><LogOut className="w-3.5 h-3.5" /> Check Out</> : <><LogIn className="w-3.5 h-3.5" /> Check In</>}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Tasks', value: stats.total, color: 'text-indigo-600', bg: 'bg-indigo-50 dark:bg-indigo-900/20', icon: CheckSquare },
          { label: 'Completed', value: stats.completed, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/20', icon: CheckCircle2 },
          { label: 'In Progress', value: stats.inProgress, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20', icon: Clock },
          { label: 'Overdue', value: stats.overdue, color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/20', icon: AlertCircle },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="card overflow-hidden">
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-700">
              {urgentTasks.map(task => {
                ...
                return (
                  <Link key={task.id} href="/employee/tasks" className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/30">
        ...
        <div className="card overflow-hidden">
          <div className="p-4 pb-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
            ...
            <Link href="/employee/tasks" className="text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1">
            ...
            {recentTasks.length === 0 ? (
              ...
              return (
                <Link key={task.id} href="/employee/tasks" className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/30">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-slate-900 dark:text-white truncate">{task.title}</p>
                    <p className="text-xs text-slate-400 mt-0.5">{formatDate(task.deadline)}</p>
                  </div>
